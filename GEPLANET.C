...
VOID FUNC check_spy(VOID)
{
    ...
    if (plptr->spyowner[0] != 0) {
        ...
        spycnt = (SHORT)plptr->items[I_SPY].qty;
...
VOID FUNC multiply(VOID)
{
    ...
    for (i = 0; i < NUMITEMS; ++i) {
        ...
        sprintf(gechrbuf, "Pl prod %d %d %s %s max:%ld f:%g hrs:%f qty:%f maxf:%f cnt:%ld", plptr->xsect, plptr->ysect, plptr->userid, item_name[i], max, fact, hrs, qty, maxf, temp);

        logthis(gechrbuf);

        if (plptr->items[i].qty <= (ULONG)max && temp >= (ULONG)max) {
...
/**************************************************************************
 ** look up sector. If not found make one... xsect and ysect are used    **
 **************************************************************************/
SHORT FUNC getsector(COORD *sect)
{
    SHORT i;
...
SHORT FUNC xgetsector(COORD *sect,SHORT wormy)
{
    SHORT  p, i, k, j;
    DOUBLE ddist;
    byte   l_tooclose;
    ...
    if (!gesdb(GEGET, &pkey, &sector)) {
        ...
        logthis(spr("GE: Ins sect X:%d Y:%d Pl:%d", pkey.xsect, pkey.ysect, p));
            ...
            for (i = 0; i < p; ++i) {
                if (sector.ptab[i].type == PLTYPE_PLNT) {
                    ...
                    if (rndm(3.99) > 3) {
                        ...
                        planet.items[I_MEN].qty   = (ULONG)rndm(50000.0);
                        planet.items[I_MEN].rate  = 5 + (USHORT)rndm(25.0);
                        planet.items[I_FOOD].qty  = (ULONG)rndm(3200.0);
                        planet.items[I_FOOD].rate = 15 + (USHORT)rndm(15.0);
                    }
...
VOID FUNC build_plan_1(SHORT idx)
{
    setmem(&planet, sizeof(GALPLNT), 0);
...
VOID FUNC build_plan_2(SHORT idx)
{
    setmem(&planet, sizeof(GALPLNT), 0);
...
VOID FUNC build_other(SHORT idx)
{
    setmem(&planet, sizeof(GALPLNT), 0);
...
VOID FUNC build_worm(SHORT idx)
{
    setmem(&worm, sizeof(GALWORM), 0);
 ...
 ** look up planet                                                       **
 **************************************************************************/
SHORT FUNC getplanet(COORD *sect,SHORT plnt)
{
    pkey.xsect = coord1(sect->xcoord);
    pkey.ysect = coord1(sect->ycoord);
    pkey.plnum = plnt;
 ...
 **************************************************************************/
SHORT FUNC neutral(COORD *coord)
{
